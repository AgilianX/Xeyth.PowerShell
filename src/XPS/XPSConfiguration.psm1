...
function XPSRequire {
    ...
    param(
        ...
        [Parameter(Mandatory = $false)][string]$helpUrl,
        [Parameter(Mandatory = $false)][bool]$ExcludeInstallInstructions = $false
    )

    if ($condition) { return }

    $errorMessage = "[⚠️ MISSING] $message"
    if (-not $ExcludeInstallInstructions -and $null -ne $helpUrl) {
        $errorMessage += "`n`nInstall: $helpUrl`n"
    }

    throw $errorMessage
}

function XPSLoadScripts {
    [CmdletBinding()]
    param (
