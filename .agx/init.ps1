...
if (Test-Path $installHooksPath) { & $installHooksPath }
else { Write-Host '⚠️ Warning: Hook installation script not found' -ForegroundColor Yellow }
...
if (Test-Path $installAliasesPath) { & $installAliasesPath }
else { Write-Host '⚠️ Warning: Alias installation script not found' -ForegroundColor Yellow }
...
if ($readmePath) { Write-Host "📚 See $readmePath for more information on repository configuration" -ForegroundColor Cyan }
else { Write-Host "📚 See $partialReadmePath for more information on repository configuration" -ForegroundColor Cyan }
